...
def calculate_rank_value(data, weight_x=0.5, weight_y=0.5):
    ...
    """
    # Rank value is calculated using a weighted sum of the number of songs and popularity
...
def get_top_artists(data, top_n=5):
    ...
    """
    # Group by artist name and calculate the average rank value for each artist
    data['avg_rank_value'] = data.groupby('ArtistName')['rank_value'].transform('mean')
    
    # Sort by the average rank value and retrieve the top N artists
    top_artists = data.sort_values(by='avg_rank_value', ascending=False).drop_duplicates('ArtistName').head(top_n)
    
    # Return the rows for the top artists
...
def display_table(data):
    ...
    # Replace NaN values with 'Null' to make missing values explicit
...
def plot_data(data):
    ...
    """
    # Pivot the data to have years as index and artist names as columns with rank values
    pivot_data = data.pivot(index='Year', columns='ArtistName', values='rank_value')
    
    # Plot the rank values for each artist over time
...
def get_top_5_artists_by_year_range(start_year, end_year):
    ...
    # Retrieve the artist data for the given year range
...
def plot_top_5_artists_rank_values(start_year, end_year):
    ...
    Parameters:
        ...
        end_year (int): The ending year.
    """
    # Retrieve and process the top 5 artists' data for the given year range
    top_5_artists = get_top_5_artists_by_year_range(start_year, end_year)
    
    if top_5_artists is None:
        return

    # Display the top 5 artists' data
    ...
    # Plot the rank values for each of the top 5 artists
...
# Get the top artists and plot their rank values
plot_top_5_artists_rank_values(2000, 2010)
